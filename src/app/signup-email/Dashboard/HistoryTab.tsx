...
const HistoryTab = () => {
    ...
    const { data, isLoading, error } = useSWR(`/api/emails/partners/all-emails?rows=${rows}&page=${currentPage}`, fetcher, { refreshInterval: 1000 });
                                    ...
                                    <tbody className="bg-white dark:bg-neutral-900 divide-y divide-neutral-200 dark:divide-neutral-800">
                                        {data.data.records.map((item: EmailsType, index: number) => (
                                            <tr key={item.id}>
                                                ...
                                                <span>{item.brand_domain !== null ? item.brand_domain : '...'}</span>
                                            </td> */}
                                                <td className="px-6 py-4 text-center whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-400">
                                                    <span>{item.note !== null ? item.note.substring(50) : "..."}</span>
                                                </td>
                                                <td className="px-6 py-4 text-center whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-400">
