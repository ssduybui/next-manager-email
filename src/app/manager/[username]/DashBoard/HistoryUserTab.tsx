...
const HistoryUserTab: React.FC<Props> = ({ viewer }) => {
    ...
    const { data, isLoading, error } = useSWR(`/api/emails/partners/all-emails?viewer=${viewer}&rows=${rows}&page=${currentPage}`, fetcher, { refreshInterval: 1000 });
                                        ...
                                        {data.data.records.map((item: EmailsType, index: number) => (
                                            <tr key={item.id}>
                                                ...
                                                <td className="px-6 py-4 text-center whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-400">
                                                    <span title={item.note !== null ? item.note : "..."}>{item.note !== null ? item.note.substring(0,25) : "..."}</span>
                                                </td>
                                                <td className="px-6 py-4 text-center whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-400">
                                                    <span>{ConvertDateTime(item.updated_at)}</span>
                                                </td>
