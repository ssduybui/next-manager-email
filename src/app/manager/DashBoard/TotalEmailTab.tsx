...
const TotalEmailPage = () => {
    ...
    const { data, isLoading, error } = useSWR(`/api/emails/manager/all-emails?rows=${rows}&page=${currentPage}`, fetcher, { refreshInterval: 1000 });
                            ...
                            {!error && !isLoading ?
                                <>
                                    <tbody className="bg-white dark:bg-neutral-900 divide-y divide-neutral-200 dark:divide-neutral-800">
                                        {data.data.records.map((item: EmailsType, index: number) => (
                                            <tr key={item.id}>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center w-96 lg:w-auto max-w-md overflow-hidden">
                                                        <div className="ms-4 flex-grow text-center">
                                                            <Link
                                                                href={`https://www.google.com/search?query=${item.brand_name}`}
                                                                target="_blank"
                                                ...
                                                {/* <td className="px-6 py-4 text-center whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-400">
                                                <span>{item.brand_domain !== null ? item.brand_name : '...'}</span>
                                            </td> */}
                                                <td className="px-6 py-4 text-center whitespace-nowrap">
                                                    <span className={`px-2 text-xs leading-5 font-medium rounded-full lg:text-sm ${item.state === "Success" ? 'bg-teal-100 text-teal-900' : `${item.state === 'Pending' ? 'bg-red-100 text-red-900' : ''}`}`}>
                                                        {item.state !== null ? item.state : '...'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-center whitespace-nowrap">
                                                    <span className="px-2 text-xs leading-5 font-medium text-teal-900 lg:text-sm">
                                                        {item.employees !== null ? item.employees : '...'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-center whitespace-nowrap">
                                                    <span className="px-2 text-xs leading-5 font-medium text-teal-900 lg:text-sm">
                                                        {item.updated_at !== null ? ConvertDateTime(item.updated_at) : '...'}
                                                    </span>
                                                </td>
                                            </tr>
                                        ))}
